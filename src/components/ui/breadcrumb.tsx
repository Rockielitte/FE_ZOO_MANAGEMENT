...
export const BreadcrumbItem = React.forwardRef<HTMLLIElement, BreadcrumbItemProps>(
  ({ children, className, isCurrentPage, lastChild, separator, addSeparator, ...props }, forwardedRef) => {
    const validChildren = getValidChildren(children)
    ...
    return (
      <li className={cn('inline-flex items-center', className)} {...props} ref={forwardedRef}>
        {clones}
        {!lastChild && addSeparator && <BreadcrumbSeparator>{separator}</BreadcrumbSeparator>}
      </li>
    )
  }
)
...
export const BreadcrumbLink = React.forwardRef<HTMLAnchorElement, BreadcrumbLinkProps>(
  ({ className, as: asComp, isCurrentPage, ...props }, forwardedRef) => {
    ...
    return (
      <Comp
        className={cn(
          'text-sm font-medium underline-offset-4 transition-colors aria-[current]:opacity-60 [&:not([aria-current])]:hover:underline',
          className
        )}
